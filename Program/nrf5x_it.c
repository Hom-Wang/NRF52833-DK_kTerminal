...
extern TIMER_ChannelInitTypeDef htimer_cc;
...
void SERIAL_UARTx_IRQHANDLER( void )
{
    if (UART_EVENTS_RXDRDY(hserial.Instance) != RESET)
    {
...
{
    if (TIMER_EVENTS_COPPARE(htimer.Instance, htimer_cc.Channel) != RESET)
    {
        TIMER_EVENTS_COPPARE(htimer.Instance, htimer_cc.Channel) = RESET;
        TIMER_TASKS_CLEAR(htimer.Instance) = SET;
        htimer_cc.EventCallback();
    }
}
