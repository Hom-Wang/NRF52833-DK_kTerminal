...
float dwt_convertrawtemperature(uint8 raw_temp)
{
    ...
    assert(pdw1000local->otp_mask & DWT_READ_OTP_TMP);
#endif
    // the User Manual formula is: Temperature (degC) = ( (SAR_LTEMP - OTP_READ(Vtemp @ 23degC) ) x 1.14) + 23
 ...
 * output parameters:
 *
 * returns: temperature sensor value in DW IC temperature units (1.14degC steps)
 */
